...
public class Biblioteca implements InterfazBiblioteca {
	...
	/**
	 * El nuevo constructor recibirá como argumentos los nombres del fichero de clientes y del
	 * fichero de libros con los que se ha de inicializar la biblioteca, es decir, en lugar de crear una biblioteca
	 * sin datos previos, lo haremos a partir de datos que tendremos almacenados en ficheros de texto(clientes.txt y libros.txt)
	 * 
	 * - Los nombres de los clientes en el fichero de texto de clientes.txt los encontraréis en diferenteslíneas.
	 * 
	 * - En el fichero libros.txt, cada línea contendrá la información concerniente a un libro, estando
	 * los distintos datos separados por asterisco (“*”) y los personajes vendrán dados separados por 
	 * comasordenados según la importancia de su papel en la película.
	 * 
	...
	public Biblioteca(String nombreFicheroClientes, String nombreFicheroLibros) {
		...
		try (Scanner sc = new Scanner(Path.of("./src/recursos/files/" + nombreFicheroLibros).toFile())) {
			...
			while (sc.hasNextLine()) {
				...
				if (!linea.isEmpty()) {
					String[] arrayLibros = linea.split("\\*");
					List<String> personajes = Arrays.asList(arrayLibros[3].split(",\\s*"));
					Libro libro = new Libro(arrayLibros[0], arrayLibros[1], arrayLibros[2], personajes);
	...
	public Libro sacarLibro(String tit) throws BibliotecaException {
		...
		/**
		 * A modo de ejemplo, en el método “prestarLibro”, un cliente no válido, un título no disponible o
		 * cualquier otra situación anómala deben producir excepciones, en lugar de devolver un valor
		 * booleano a falso, que era lo que la mayoría hacíais hasta este momento. A modo de ejemplo, nuestro código antes podía ser:
		 * 
		 * 		if (libro==null) return false;
		 * 
		 * Ahora lanzaremos una excepción, como la siguiente:
		 * 
		 * 		if (libro==null)
		 * 			throw new BibliotecaException("Libro no disponible");
		 */
